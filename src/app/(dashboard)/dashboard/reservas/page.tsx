  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-50">Reservas</h1>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {reservations.map((r) => (
              <tr key={r.id} className="border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800">
                <td className="px-5 py-4">
                  <Link href={`/dashboard/reservas/${r.id}`} className="block">
                    <p className="font-medium text-slate-900 dark:text-slate-50">{r.property.address}</p>
                  </Link>
                </td>
                <td className="px-5 py-4 text-slate-700 dark:text-slate-300">{r.renter.name}</td>
                <td className="px-5 py-4 text-slate-600 dark:text-slate-400">{formatDate(r.reservationDate)}</td>
                <td className="px-5 py-4 text-slate-600 dark:text-slate-400">{formatEUR(r.amount)}</td>
                <td className="px-5 py-4">
                  <span
            ...
            {reservations.length === 0 && (
              <tr>
                <td colSpan={5} className="px-5 py-10 text-center text-slate-400 dark:text-slate-500">
                  ...
                  <Link href="/dashboard/reservas/nueva" className="text-slate-900 dark:text-slate-50 underline">
                    Crear la primera
                  </Link>
                </td>
              </tr>
