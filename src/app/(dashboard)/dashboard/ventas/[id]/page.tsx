...
export default async function VentaDetailPage({
  ...
  return (
    <div className="space-y-8">
      <div>
        <Link href="/dashboard/ventas" className="text-sm text-slate-500 hover:text-slate-900">
          ← Volver a ventas
        </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card title="Documentos de la propiedad">
            <div className="space-y-2">
              {sale.property.documents.length === 0 && (
                <p className="text-sm text-slate-400">Sin documentos cargados (escritura, etc.).</p>
              )}
        ...
        <div className="space-y-6">
          <Card title="Estado de la venta">
            ...
            {sale.saleDate && (
              <p className="mt-3 text-xs text-slate-500">Vendida el {formatDate(sale.saleDate)}</p>
            )}
          </Card>
          {sale.notes && (
            <Card title="Notas">
