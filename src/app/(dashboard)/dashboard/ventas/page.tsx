              ...
              <tr key={s.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                <td className="px-5 py-4">
                  <Link href={`/dashboard/ventas/${s.id}`} className="block">
                    <p className="font-medium text-slate-900">{s.property.address}</p>
                    <p className="text-xs text-slate-500">{s.property.type}</p>
                  </Link>
                </td>
                <td className="px-5 py-4 text-slate-700">{s.buyer ? s.buyer.name : "—"}</td>
                <td className="px-5 py-4 font-medium text-slate-900">{formatEUR(s.price)}</td>
                <td className="px-5 py-4">
                  <span
            ...
            {sales.length === 0 && (
              <tr>
                <td colSpan={4} className="px-5 py-10 text-center text-slate-400">
                  ...
                  <Link href="/dashboard/ventas/nueva" className="text-slate-900 underline">
                    Crear la primera
                  </Link>
                </td>
              </tr>
