...
export default function NuevaPropiedadPage() {
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-50">Nueva propiedad</h1>
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-6">
        <Section title="Datos generales">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Se ofrece en</label>
              <select
                name="listingType"
...
function Checkbox({ name, label }: { name: string; label: string }) {
  return (
    <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
      <input name={name} type="checkbox" className="rounded border-slate-300 dark:border-slate-700" />
      {label}
    </label>
  );
}
